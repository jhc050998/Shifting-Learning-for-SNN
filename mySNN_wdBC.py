...
def wdBC():
    ...
    for f in range(5):  # 5-fold cross validation
        ...
        # Network layout
        ly1 = mySNN.SNNLayer(inCh=30, outCh=40)
        ly2 = mySNN.SNNLayer(inCh=40, outCh=2)
        ...
        # Training process
        epoch_num = 600
        lr_start, lr_end = 1e-2, 1e-3  # decaying learning rate for shifting timings
...
def wdBC_mly():
    ...
    for f in range(5):  # 5-fold cross validation
        ...
        # Network layout
        ly1 = mySNN.SNNLayer(inCh=30, outCh=40)
        ly2 = mySNN.SNNLayer(inCh=40, outCh=40)
        ly3 = mySNN.SNNLayer(inCh=40, outCh=2)
        ...
        # Training process
        epoch_num = 400  # 400
        lr_start, lr_end = 1e-2, 1e-3  # decaying learning rate for shifting timings -2-3
        lr_decay = (lr_end / lr_start) ** (1.0 / epoch_num)
        lr_Etp = 1e-4
        ...
        for epoch in range(epoch_num):
            ...
            for bi in range(bn):
                ...
                # Shifting Learning
                z3_lo, z_tar = torch.softmax(z3, dim=1), torch.softmax(torch.exp(1.0-tar_2), dim=1)
                delta3 = z3_lo - z_tar
